...
export class HomePage extends BasePage {
  ...
  private get mongodbQueryMessage(): Locator {
    return this.page.getByTestId("mongodb-query-message");
  }
  ...
  private async interactWithMongodbQueryMessage(): Promise<Locator> {
    const isVisible = await waitForElementToBeVisible(this.mongodbQueryMessage);
    if (!isVisible) throw new Error("MongoDB query message is not visible!");
    return this.mongodbQueryMessage;
  }
  ...
  async selectDatabaseType(type: "mongodb"): Promise<void> {
  ...
  async waitForMongoDBQuery(): Promise<boolean> {
    try {
      // Wait for at least one SQL query message to appear (using count to avoid strict mode violation)
      await this.page.waitForFunction(
        () => {
          const messages = document.querySelectorAll('[data-testid="mongodb-query-message"]');
          return messages.length > 0;
        },
        { timeout: 30000 }
      );
   ...
   */
  async isMongoDBQueryMessageVisible(): Promise<boolean> {
    try {
      const count = await this.MongoDBQueryMessage.count();
      return count > 0;
    } catch {
      return false;
    }
   ...
   */
  async verifyMongoDBQueryContains(expectedText: string): Promise<boolean> {
    try {
      const sqlMessage = await this.interactWithMongoDBQueryMessage();
  ...
  async getMessageCount(): Promise<number> {
    try {
      ...
      const sqlCount = await this.MongoDBQueryMessage.count();
  ...
  async waitForCompleteQueryResponse(): Promise<boolean> {
    try {
      ...
      const mongoQueryVisible = await this.waitForMongoDBQuery();
      if (!mongoQueryVisible) return false;
  ...
  async ensureDatabaseConnected(apiCall: ApiCalls): Promise<boolean> {
    ...
    const response = await apiCall.connectDatabase(mongodbUrl);
