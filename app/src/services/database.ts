...
export class DatabaseService {
   ...
   * Accepts CSV files or JSON schema definitions
   */
  ...
  static async connectDatabase(config: {
    ...
    try {
      ...
      const connectionUrl = `${protocol}://${encodeURIComponent(config.username)}:${encodeURIComponent(config.password)}@${config.host}:${config.port}/${encodeURIComponent(config.database)}?authSource=admin`;
