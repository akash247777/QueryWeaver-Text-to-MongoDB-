...
import { useChat, ChatMessageData } from "@/contexts/ChatContext";
    ...
    try {
      ...
      for await (const message of ChatService.streamQuery({
        ...
        if (message.type === 'status' || message.type === 'reasoning' || message.type === 'reasoning_step') {
          ...
          const stepText = String(message.content || message.message || '');
        ...
        } else if (message.type === 'mongodb_query') {
          // Store MongoDB query to display - handle both string and object
          const rawQuery = message.mongodb_query || message.content || message.message || '';
          mongodbQuery = typeof rawQuery === 'object' ? JSON.stringify(rawQuery, null, 2) : String(rawQuery);
          ...
          analysisInfo = {
            ...
            isValid: message.is_query_translatable !== undefined ? message.is_query_translatable : (message.is_valid !== undefined ? message.is_valid : true)
          };
          ...
          const responseContent = String(message.message || message.content || '').trim();
          finalContent = responseContent;
        } else if (message.type === 'followup_questions') {
          // Follow-up questions when query is unclear or off-topic
          const followupContent = String(message.message || message.content || '').trim();
          finalContent = followupContent;
        } else if (message.type === 'error') {
          // Handle error
          const errorMsg = String(message.message || message.content || 'Unknown error occurred').trim();
          toast({
            title: "Query Failed",
            description: errorMsg,
            variant: "destructive",
          });
          finalContent = `Error: ${errorMsg}`;
          ...
          const confirmationMessage: ChatMessageData = {
            ...
            confirmationData: {
              mongodbQuery: String(message.mongodb_query || ''),
              operationType: String(message.operation_type || 'UNKNOWN').toUpperCase(),
      ...
      // Add MongoDB query message with analysis info
      if (mongodbQuery !== undefined || Object.keys(analysisInfo).length > 0) {
        const mongodbMessage: ChatMessageData = {
          id: (Date.now() + 2).toString(),
          type: "mongodb-query", // For UI classification
          content: mongodbQuery,
          analysisInfo: analysisInfo,
          timestamp: new Date(),
        };
        setMessages(prev => [...prev, mongodbMessage]);
      }
  ...
  const handleConfirmDestructive = async (messageId: string) => {
    ...
    try {
      ...
      for await (const message of ChatService.streamConfirmOperation(
        selectedGraph.id,
        {
          mongodb_query: confirmMessage.confirmationData.mongodbQuery,
        ...
        if (message.type === 'status' || message.type === 'reasoning' || message.type === 'reasoning_step') {
          ...
          let errorMsg = String(message.message || message.content || 'Unknown error occurred');
          ...
          } else if (errorMsg.includes('MongoDB query execution error:')) {
            // Strip the "MongoDB query execution error:" prefix
            errorMsg = errorMsg.replace(/^(MongoDB) query execution error:\s*/i, '');
          ...
          const refreshContent = String(message.message || message.content || '');
  ...
  const handleCancelDestructive = (messageId: string) => {
    ...
    setMessages(prev => [
      ...prev,
      {
        id: `cancel-${Date.now()}`,
        type: 'ai',
        content: 'Operation cancelled. The destructive query was not executed.',
        timestamp: new Date(),
      }
    ]);
