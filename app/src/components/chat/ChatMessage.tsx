...
import { ChatMessageData } from '@/contexts/ChatContext';



interface ChatMessageProps extends Omit<ChatMessageData, 'id' | 'timestamp'> {
...
const ChatMessage = ({ type, content, steps, queryData, analysisInfo, confirmationData, progress, user, onConfirm, onCancel }: ChatMessageProps) => {
                    ...
                    {confirmationData?.mongodbQuery && (
                      <div className="bg-background border border-border rounded p-3 overflow-x-auto">
                        <pre className="text-sm font-mono text-foreground whitespace-pre-wrap break-words overflow-wrap-anywhere">
                          <code className="language-json">{confirmationData.mongodbQuery}</code>
                        </pre>
                      </div>
                    )}
                  </div>
  ...
  if (type === 'mongodb-query') {
    const safeContent = content || '';
    const hasMongoDB = safeContent.trim().length > 0;
    const isValid = analysisInfo?.isValid !== false; // Default to true if not specified

    return (
      <div className="px-6" data-testid="mongodb-query-message">
        <div className="flex gap-3 mb-6 items-start">
          <Avatar className="w-8 h-8 flex-shrink-0">
            <AvatarFallback className="bg-primary text-primary-foreground text-xs font-bold">
              QW
              ...
              <CardContent className="p-4">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className={`text-base font-semibold ${isValid ? 'text-primary' : 'text-warning'}`}>
                    {hasMongoDB ? 'Generated MongoDB Query' : 'Query Analysis'}
                  </span>
                </div>
                  ...
                  <div className="space-y-2 text-sm">
                    {analysisInfo?.explanation && (
                      <div className="bg-background/50 p-2 rounded">
                        ...
                        <p className="text-foreground mt-1">{String(analysisInfo.explanation)}</p>
                      </div>
                    )}
                    {analysisInfo?.missing && (
                      <div className="bg-background/50 p-2 rounded">
                        <span className="font-semibold text-warning">Missing Information:</span>
                        <p className="text-foreground mt-1">{String(analysisInfo.missing)}</p>
                      </div>
                    )}
                    {analysisInfo?.ambiguities && (
                      <div className="bg-background/50 p-2 rounded">
                        <span className="font-semibold text-warning">Ambiguities:</span>
                        <p className="text-foreground mt-1">{String(analysisInfo.ambiguities)}</p>
                      </div>
                    )}
                  </div>
                )}
                      ...
                      <table className="text-sm border-collapse" data-testid="results-table" style={{ width: '100%', maxWidth: '100%', tableLayout: 'auto', display: 'table' }}>
                        <thead className="sticky top-0 bg-card z-10">
                          <tr className="border-b border-border">
                            {queryData[0] && typeof queryData[0] === 'object' ? (
                              ...
                              <th className="text-left px-3 py-2 text-muted-foreground font-semibold bg-card">Result</th>
                            )}
                          </tr>
                        </thead>
                        <tbody>
                          {queryData.map((row, index) => (
                            <tr key={index} className="border-b border-border hover:bg-muted">
                              {row && typeof row === 'object' ? (
                                ...
                                <td className="px-3 py-2 text-foreground break-words">{String(row)}</td>
                              )}
                            </tr>
                          ))}
                        </tbody>
