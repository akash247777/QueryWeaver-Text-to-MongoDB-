      ...
      <DialogContent className="sm:max-w-[500px] max-h-[90vh] overflow-y-auto bg-card border-border">
        <DialogHeader>
          ...
          <DialogDescription className="text-sm text-muted-foreground">
            Connect to your MongoDB database using a connection URL.{" "}
            <a
          ...
          <div className="space-y-2">
            ...
            <Select onValueChange={setSelectedDatabase} value={selectedDatabase}>
              ...
              <SelectContent className="bg-card border-border">
                <SelectItem value="mongodb" className="focus:bg-purple-500/20 focus:text-foreground" data-testid="mongodb-option">
            ...
            <div className="mt-4 space-y-2 max-h-[220px] overflow-y-auto border border-border rounded-md p-3 bg-muted/30">
              {connectionSteps.map((step, index) => (
                <div key={index} className="flex items-start gap-2 text-sm">
                  ...
                  <span className={`flex-1 ${step.status === 'error' ? 'text-red-400' : 'text-card-foreground'
                    }`}>
                    {step.message}
                  </span>
                </div>
