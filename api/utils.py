"""Utility functions for the text-to-MongoDB query API."""
import json
        ...
        for offset, batch_response in enumerate(response):
            ...
            if isinstance(batch_response, Exception):
                ...
                if content is not None:
                    table_info[table_name]["description"] = content.strip()
                else:
                    table_info[table_name]["description"] = table_name
...
def generate_db_description(
    ...
    return description.strip() if description else f"{db_name} database."
