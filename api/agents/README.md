...
This module contains various AI agents for the QueryWeaver application. Each agent is responsible for a specific task in the query processing pipeline.
...
- **Purpose**: Provides taxonomy classification and clarification for MongoDB queries
- **Key Method**: `get_answer()` - Generates clarification questions for MongoDB queries
- **Features**: Filter analysis, user-friendly clarifications

### ResponseFormatterAgent (`response_formatter_agent.py`)
- **Purpose**: Formats MongoDB query results into user-readable responses
- **Key Method**: `format_response()` - Converts raw MongoDB results to natural language
...
# Use agents
analysis = analysis_agent.get_analysis(query, collections, db_description)
relevancy = relevancy_agent.get_answer(question, database_desc)
response = formatter_agent.format_response(query, mongodb_query, results, db_description)
```
