...
class BaseAgent:  # pylint: disable=too-few-public-methods
    ...
    def __init__(self, queries_history: list = None, result_history: list = None):
        ...
        self.messages = []
        if queries_history and result_history:
            # We skip the latest query because it's current
    ...
    try:
        ...
        for json_str in reversed(json_blocks):
            try:
                ...
                if "is_query_translatable" in analysis and "mongodb_query" in analysis:
                    return analysis
