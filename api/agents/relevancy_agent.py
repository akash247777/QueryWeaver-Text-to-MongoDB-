...
class RelevancyAgent(BaseAgent):
    ...
    async def get_answer(self, user_question: str, database_desc: dict) -> dict:
        ...
        # Use simple string formatting to create the prompt
        user_prompt = RELEVANCY_PROMPT.format(
            QUESTION_PLACEHOLDER=user_question,
            DB_PLACEHOLDER=json.dumps(database_desc),
        )
        
        # We append the specific instruction as the final user message
        # we only send the current history + the current prompt
        ...
        # Simple JSON extraction for relevancy since it doesn't match the translatable structure
        try:
            json_start = answer.find("{")
            json_end = answer.rfind("}") + 1
            if json_start >= 0 and json_end > json_start:
                return json.loads(answer[json_start:json_end])
            return {"status": "Off-topic", "reason": "Could not parse JSON", "suggestions": []}
        except Exception:
            return {"status": "Off-topic", "reason": "Parsing error", "suggestions": []}
