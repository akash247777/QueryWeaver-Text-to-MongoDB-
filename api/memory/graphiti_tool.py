...
# Conditionally import Gemini components
try:
    from graphiti_core.embedder.gemini import GeminiEmbedder, GeminiEmbedderConfig
    from graphiti_core.llm_client.gemini_client import GeminiClient
    from graphiti_core.cross_encoder.gemini_reranker_client import GeminiRerankerClient
    GEMINI_COMPONENTS_AVAILABLE = True
except ImportError:
    GEMINI_COMPONENTS_AVAILABLE = False
...
class MemoryTool:
    ...
    async def update_user_information(self, conversation: Dict[str, Any], history: Tuple[List[str], List[str]]) -> bool:
        ...
        if conversation.get('mongodb_query'):
            conv_text += f"Query: {conversation['mongodb_query']}\n"
    ...
    async def save_query_memory(self, query: str, mongodb_query: str, success: bool, error: Optional[str] = None) -> bool:
        """
        Save individual query memory directly to the database node.
        
        Args:
            query: The user's natural language query
            mongodb_query: The generated MongoDB query
        ...
        try:
            ...
            # Check if Query node with same user_query and mongodb_query already exists
            relationship_type = "SUCCESS" if success else "FAILED"
            
            # Escape quotes in the query and MongoDB for Cypher
            escaped_query = query.replace("'", "\\'").replace('"', '\\"')
            escaped_mongodb = mongodb_query.replace("'", "\\'").replace('"', '\\"')
            escaped_error = error.replace("'", "\\'").replace('"', '\\"') if error else ""
            embeddings = Config.EMBEDDING_MODEL.embed(escaped_query)[0]

            # First check if a Query node with the same user_query and mongodb_query already exists
            check_query = f"""
            MATCH (db:Entity {{uuid: "{database_node_uuid}"}})
            MATCH (db)-[r]->(q:Query)
            WHERE q.user_query = "{escaped_query}" AND q.mongodb_query = "{escaped_mongodb}"
            ...
            if check_result[0]:  # If records exist
                logging.info("Query with same user_query and mongodb_query already exists, skipping creation")
                return True
            ...
            if similar_queries:
                ...
                if successful_queries:
                    ...
                    for i, query_data in enumerate(successful_queries, 1):
                        ...
                        memory_context += f"   Successful MongoDB Query: {query_data.get('mongodb_query', '')}\n\n"

                if failed_queries:
                    memory_context += "FAILED QUERIES (Avoid these patterns):\n"
                    for i, query_data in enumerate(failed_queries, 1):
                        memory_context += f"{i}. Query: \"{query_data.get('user_query', '')}\"\n"
                        memory_context += f"   Failed MongoDB Query: {query_data.get('mongodb_query', '')}\n"
...
def create_graphiti_client(falkor_driver: FalkorDriver) -> Graphiti:
    ...
    elif Config.GOOGLE_FLAG and GEMINI_COMPONENTS_AVAILABLE:
        # Use Gemini embedder, Gemini client, and Gemini reranker for Google AI Studio
        embedding_model_name = extract_embedding_model_name(Config.EMBEDDING_MODEL_NAME)
        completion_model_name = extract_embedding_model_name(Config.COMPLETION_MODEL)
